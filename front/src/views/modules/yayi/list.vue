                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yayi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yakejiuzhenguanli/upload/yayiMuBan.xls"
                        >批量导入牙医数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yayi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="yayiUploadSuccess"
                                :on-error="yayiUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yayi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yayi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('yayi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yayi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('yayi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('yayi','修改')" type="success" size="mini" @click="jinqiyong(scope.row.id,scope.row.jinyongTypes)">{{scope.row.jinyongTypes == 1?'禁用账户':'启用账户'}}</el-button>

                            <el-button v-if="isAuth('yayi','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                zhiweiTypes : null,
                yayiContent : null,
                yayiShanchangContent : null,
                yayiRongyuContent : null,
                jinyongTypes : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yayi',//当前表表名,
                        sumColum : 'yayi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yayi_name',//分组字符串字段
                        // types : 'yayi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['yayiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            yayiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yayi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            jinqiyong(id,jinyongTypes) {
                ...
                _this.$confirm(msg, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"}).then(() => {
                    _this.$http({url: "yayi/update",method: "post",data: {id:id,jinyongTypes:jinyongTypes,}}).then(({data}) => {
                        if(data && data.code === 0){
