        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '订单编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.yayiOrderUuidNumber" placeholder="订单编号" clearable></el-input>
                     </el-form-item>
                         
                     <el-form-item :label="contents.inputTitle == 1 ? '预约类型' : ''">
                        <el-select v-model="searchForm.yayiOrderYuyueTypes" placeholder="请选择预约类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in yayiOrderYuyueTypesSelectSearch"
                               v-bind:key="index"
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '预约状态' : ''">
                        <el-select v-model="searchForm.yayiOrderTypes" placeholder="请选择预约状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in yayiOrderTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '牙医姓名' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.yayiName" placeholder="牙医姓名" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yayiOrder','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yakejiuzhenguanli/upload/yayiOrderMuBan.xls"
                        >批量导入牙医挂号数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yayiOrder','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="yayiOrderUploadSuccess"
                                :on-error="yayiOrderUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('yayiOrder','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入牙医挂号数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yayiOrder','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yayiOrder.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yayiOrderUuidNumber"
                                   header-align="center"
                                   label="订单编号">
                        <template slot-scope="scope">
                            {{scope.row.yayiOrderUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="yayiOrderYuyueTypes"
                                      header-align="center"
                                      label="预约类型">
                        <template slot-scope="scope">
                            {{scope.row.yayiOrderYuyueValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('yayiOrder','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yayiOrder','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('yayiOrder','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
            <el-button v-if="isAuth('yayiOrder','订单') && scope.row.yayiOrderTypes == 101 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-sold-out" size="mini" @click="refund(scope.row.id)">取消预约</el-button>
            <el-button v-if="isAuth('yayiOrder','订单') && scope.row.yayiOrderTypes == 103 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="openCommentModel(scope.row.id)">评价</el-button>
            <el-button v-if="isAuth('yayiOrder','订单') && scope.row.yayiOrderTypes == 101
&& sessionTable=='yayi'" type="primary" icon="el-icon-reading" size="mini"
...
<script>
    ...
    //级联表下拉框搜索条件
              ...
              jinyongTypesSelectSearch : [],
    //当前表下拉框搜索条件
              yayiOrderYuyueTypesSelectSearch : [],
              yayiOrderTypesSelectSearch : [],
            form:{
                id : null,
                yayiOrderUuidNumber : null,
                yayiId : null,
                yonghuId : null,
                yayiOrderYuyueTypes : null,
                insertTime : null,
                yayiOrderTime : null,
                yayiOrderTypes : null,
                createTime : null,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '订单编号': "yayiOrderUuidNumber",
                     '预约类型': "yayiOrderYuyueValue",
                     '申请时间': "insertTime",
                     '预约日期': "yayiOrderTime",
                     '预约状态': "yayiOrderValue",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yayi_order',//当前表表名,
                        sumColum : 'yayi_order_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yayi_order_name',//分组字符串字段
                        // types : 'yayi_order_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.yayiUuidNumber!= '' && this.searchForm.yayiUuidNumber!= undefined) {
                    params['yayiUuidNumber'] = '%' + this.searchForm.yayiUuidNumber + '%'
                }
                     
                if (this.searchForm.yayiName!= '' && this.searchForm.yayiName!= undefined) {
                    params['yayiName'] = '%' + this.searchForm.yayiName + '%'
                }
                ...
                if (this.searchForm.yayiOrderUuidNumber!= '' && this.searchForm.yayiOrderUuidNumber!= undefined) {
                    params['yayiOrderUuidNumber'] = '%' + this.searchForm.yayiOrderUuidNumber + '%'
                }
                         
                if (this.searchForm.yayiOrderYuyueTypes!= '' && this.searchForm.yayiOrderYuyueTypes!= undefined) {
                    params['yayiOrderYuyueTypes'] = this.searchForm.yayiOrderYuyueTypes
                }
                         
                if (this.searchForm.yayiOrderTypes!= '' && this.searchForm.yayiOrderTypes!= undefined) {
                    params['yayiOrderTypes'] = this.searchForm.yayiOrderTypes
                }
        
                params['yayiOrderDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=yayi_order_yuyue_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.yayiOrderYuyueTypesSelectSearch = data.data.list;
                    }
                });
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=yayi_order_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.yayiOrderTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            yayiOrderUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yayiOrder/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
			...
			deliver(id){
				let _this = this;
				_this.$confirm(`确定要使用吗?`, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
					_this.$http({
						url: "yayiOrder/deliver?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
			...
			commentback(id){
				...
				_this.$confirm(`确定要评价吗?`, "提示", {
					...
					_this.$http({
						url: "yayiOrder/commentback?id="+_this.commentbackId+"&commentbackText="+_this.commentbackContent+"&yayiCommentbackPingfenNumber="+_this.yayiCommentbackPingfenNumber,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
								...
								onClose: () => {
									...
									//情况评分
									_this.yayiCommentbackPingfenNumber = null
								}
							});
						} else {
							//清空操作数据id
