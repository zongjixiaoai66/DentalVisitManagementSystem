        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '药单编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.yaodanUuidNumber" placeholder="药单编号" clearable></el-input>
                     </el-form-item>
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '牙医编号' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.yayiUuidNumber" placeholder="牙医编号" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yaodan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yakejiuzhenguanli/upload/yaodanMuBan.xls"
                        >批量导入药单数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yaodan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="yaodanUploadSuccess"
                                :on-error="yaodanUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yaodan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yaodan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yayiPhoto"
                               header-align="center"
                               width="200"
                               label="牙医头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.yayiPhoto">
                                <img :src="$base.url+scope.row.yayiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yaodanUuidNumber"
                                   header-align="center"
                                   label="药单编号">
                        <template slot-scope="scope">
                            {{scope.row.yaodanUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('yaodan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yaodan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('yaodan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                yaodanUuidNumber : null,
                yonghuId : null,
                yayiId : null,
                jianchaTime : null,
                bingrenzishuContent : null,
                jianchajieguoContent : null,
                yishengkaifangYaopinContent : null,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yaodan',//当前表表名,
                        sumColum : 'yaodan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yaodan_name',//分组字符串字段
                        // types : 'yaodan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.yaodanUuidNumber!= '' && this.searchForm.yaodanUuidNumber!= undefined) {
                    params['yaodanUuidNumber'] = '%' + this.searchForm.yaodanUuidNumber + '%'
                }
                ...
                params['yaodanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            yaodanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yaodan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
